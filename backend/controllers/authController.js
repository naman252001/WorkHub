...
// -----------------------------
// Login with email/password (OTP once per day)
// -----------------------------
const login = async (req, res) => {
  const { email, password } = req.body;
  ...
  try {
    ...
    // ✅ If user already logged in today, skip OTP
    if (user.lastOTPLoginDate === today) {
      const token = generateToken(user._id);
      return res.json({
        success: true,
        msg: "Logged in without OTP today",
        token,
        user,
      });
    }
...
// -----------------------------
// Verify OTP (updates lastOTPLoginDate)
// -----------------------------
const verifyOtp = async (req, res) => {
  const { email, otp } = req.body;
  ...
  try {
    ...
    // ✅ Clear OTP and save login date
    user.otpCode = null;
    user.otpExpires = null;
    user.lastOTPLoginDate = new Date().toDateString(); // mark today as logged in
    await user.save();
